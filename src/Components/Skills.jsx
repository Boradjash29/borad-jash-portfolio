...
const Skills = memo(function Skills() {
  ...
  return (
    <section id="skills" className="relative z-10 bg-[#f5f5f0] text-[#1a1a1a] py-16 sm:py-20 lg:py-28 xl:py-36 px-4 sm:px-6 lg:px-12 overflow-hidden">
          ...
          {activeTab === "capabilities" && (
            ...
            >
              ...
              <motion.p
                ...
                className="text-center text-[#1a1a1a]/40 text-xs sm:text-sm mt-6 sm:mt-8 hidden sm:block"
              >
                Hover to explore...
              </motion.p>
            </motion.div>
            ...
            >
              {techStacks.map((stack, i) => (
                <motion.div
                  ...
                  className="flex flex-col lg:flex-row lg:items-start border-b border-[#1a1a1a]/10 pb-4 sm:pb-6"
                >
                  <h3 className="font-semibold text-base sm:text-lg text-[#1a1a1a] mb-2 sm:mb-3 lg:mb-0 lg:w-1/4">
                    {stack.category}
                  </h3>
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 lg:w-3/4">
                    {stack.items.map((item) => (
                      <span
                        key={item}
                        className="px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-[#1a1a1a]/5 text-[#1a1a1a]/70 border border-[#1a1a1a]/10 hover:bg-[#1a1a1a]/10 hover:border-[#1a1a1a]/20 transition-colors cursor-default"
                      >
                        {item}
                      </span>
                    ))}
          ...
          {activeTab === "certs" && (
            ...
            >
              <div className="mb-4 sm:mb-6">
                <h3 className="text-lg sm:text-xl font-semibold text-[#1a1a1a] flex items-center gap-2">
                  <Award size={18} className="sm:w-5 sm:h-5 text-amber-500" /> Certifications
                </h3>
              </div>
              {certifications.map((cert, i) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.08 }}
                  className="flex flex-col sm:flex-row sm:items-center justify-between py-4 sm:py-5 border-b border-[#1a1a1a]/10 group"
                >
                  <div>
                    <strong className="text-[#1a1a1a] font-semibold text-sm sm:text-base group-hover:text-indigo-600 transition-colors">
                      {cert.name}
                    </strong>
                    <span className="text-[#1a1a1a]/50 text-xs sm:text-sm ml-1 sm:ml-2 block sm:inline mt-0.5 sm:mt-0">{cert.detail}</span>
                  </div>
                  <span className="tracking-mono text-[#1a1a1a]/30 text-xs sm:text-sm mt-1 sm:mt-0 flex-shrink-0">
                    {cert.date}
                  </span>
                </motion.div>
              ))}
